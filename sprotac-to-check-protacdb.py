...
###### function to check errors and split PROTACs ######
def split_protac_debug(protac_name, protac_mol, warhead_df, e3_df,
                       stats_trackers):
    """
    Debug version for large batch runs.
    - Does NOT spam Streamlit logs.
    - Fills stats_trackers dict with SMILES/name of failed compounds.
    """

    try:
        if protac_mol is None:
            stats_trackers['invalid_input'].append((protac_name, None))
            return pd.DataFrame()
        ...
        for _, warhead_row in warhead_df.iterrows():
            ...
            if matches:
                warhead_matched = True
                w_name = warhead_row.get('Compound ID', 'unknown_warhead')
                for match in matches:
                    linker_after_w = AllChem.DeleteSubstructs(protac_mol, warhead_mol)
                    protac_matches.append({
                        'Protac Mol': protac_mol,
        ...
        if not warhead_matched:
            stats_trackers['no_warhead'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()
        ...
        for _, e3_row in e3_df.iterrows():
            ...
            if matches:
                e3_matched = True
                e3_name = e3_row.get('Compound ID', 'unknown_e3')
                for match in matches:
                    linker_after_e3 = AllChem.DeleteSubstructs(protac_mol, e3_mol)
                    e3_matches.append({
                        'Protac Mol': protac_mol,
        ...
        if not e3_matched:
            stats_trackers['no_e3'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()

        # --- Merge ---
        results_df = pd.DataFrame(protac_matches)
        e3_df_tmp = pd.DataFrame(e3_matches)
        if results_df.empty or e3_df_tmp.empty:
            stats_trackers['merge_fail'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()

        final_df = pd.merge(results_df, e3_df_tmp, on='Protac Mol', how='inner')
        if final_df.empty:
            stats_trackers['merge_fail'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()

        # --- Linker equality check ---
        final_df['Final Linker Mol'] = final_df.apply(
            lambda row: (AllChem.DeleteSubstructs(row['Linker Mol_x'], row['E3 Mol'])
                         if row['Linker Mol_x'] and row['E3 Mol'] and
                            row['Linker Mol_x'].HasSubstructMatch(row['E3 Mol'])
                         else None),
            axis=1
        )
        final_df['Linker Mol Check'] = final_df.apply(
            lambda row: (AllChem.DeleteSubstructs(row['Linker Mol_y'], row['Warhead Mol'])
                         if row['Warhead Mol'] and row['Linker Mol_y'] and
                            row['Linker Mol_y'].HasSubstructMatch(row['Warhead Mol'])
                         else None),
            axis=1
        )
        final_df['Linker_check'] = final_df.apply(
            lambda r: (r['Final Linker Mol'] and r['Linker Mol Check'] and
                       mol_to_smiles(r['Final Linker Mol']) == mol_to_smiles(r['Linker Mol Check'])),
            axis=1
        )
        if not final_df['Linker_check'].any():
            stats_trackers['linker_inconsistent'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()

        final_df = final_df[final_df['Linker_check'] == True].copy()

        # --- Fragment filtering ---
        final_df['Final Linker Mol'] = final_df['Final Linker Mol'].apply(
            lambda m: Chem.MolFromSmiles(Chem.MolToSmiles(m)) if m else None)
        final_df['Num_Frags'] = final_df['Final Linker Mol'].apply(
            lambda m: len(Chem.GetMolFrags(m, asMols=True, sanitizeFrags=False)) if m else 0)
        if not any(final_df['Num_Frags'] == 1):
            stats_trackers['multi_fragment_linker'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()

        final_df = final_df[final_df['Num_Frags'] == 1].copy()

        # --- Heavy atom check ---
        final_df['HA_Protac'] = final_df['Protac Mol'].apply(lambda m: rdMolDescriptors.CalcNumHeavyAtoms(m))
        final_df['HA_Warhead'] = final_df['Warhead Mol'].apply(lambda m: rdMolDescriptors.CalcNumHeavyAtoms(m))
        final_df['HA_E3'] = final_df['E3 Mol'].apply(lambda m: rdMolDescriptors.CalcNumHeavyAtoms(m))
        final_df['HA_Link'] = final_df['Final Linker Mol'].apply(lambda m: rdMolDescriptors.CalcNumHeavyAtoms(m))
        final_df['HA_Sum'] = final_df[['HA_Warhead', 'HA_E3', 'HA_Link']].sum(axis=1)
        final_df['HA_Check'] = final_df['HA_Sum'] == final_df['HA_Protac']
        if not final_df['HA_Check'].any():
            stats_trackers['heavy_atom_mismatch'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()
        final_df = final_df[final_df['HA_Check'] == True]

        # --- Ring count check ---
        final_df['R_Protac'] = final_df['Protac Mol'].apply(lambda m: rdMolDescriptors.CalcNumRings(m))
        final_df['R_Warhead'] = final_df['Warhead Mol'].apply(lambda m: rdMolDescriptors.CalcNumRings(m))
        final_df['R_E3'] = final_df['E3 Mol'].apply(lambda m: rdMolDescriptors.CalcNumRings(m))
        final_df['R_Link'] = final_df['Final Linker Mol'].apply(lambda m: rdMolDescriptors.CalcNumRings(m))
        final_df['R_Sum'] = final_df[['R_Warhead', 'R_E3', 'R_Link']].sum(axis=1)
        final_df['R_Check'] = final_df['R_Sum'] == final_df['R_Protac']
        if not final_df['R_Check'].any():
            stats_trackers['ring_mismatch'].append((protac_name, mol_to_smiles(protac_mol)))
            return pd.DataFrame()

        # ✅ Passed all checks
        stats_trackers['success'].append((protac_name, mol_to_smiles(protac_mol)))
        return final_df

    except Exception:
        stats_trackers['exception'].append((protac_name, None))
        return pd.DataFrame()
...
def main():
    ...
    # --- Optional Debug Mode: collect failure reasons ---
    if st.checkbox("🧪 Run in Debug Mode (collect failure stats, please don't upload more than 1000 PROTACs)"):
        stats_trackers = {
            'success': [], 'no_warhead': [], 'no_e3': [], 'merge_fail': [],
            'linker_inconsistent': [], 'multi_fragment_linker': [],
            'heavy_atom_mismatch': [], 'ring_mismatch': [],
            'invalid_input': [], 'exception': []
        }

        # Progress bar setup
        total = min(len(protac_entries), 1000)  # cap for safety
        progress_bar = st.progress(0)
        status_text = st.empty()

        # Iterate over PROTACs
        for idx, (name, smiles) in enumerate(protac_entries[:1000]):
            mol = Chem.MolFromSmiles(smiles)
            split_protac_debug(name, mol, warhead_df, e3_df, stats_trackers)

            # Update progress
            progress = (idx + 1) / total
            progress_bar.progress(progress)
            status_text.text(f"🔍 Processing PROTAC {idx+1}/{total} ({progress*100:.1f}%)")

        # After all PROTACs are processed
        status_text.text("✅ Debugging completed!")
        st.success("All compounds have been processed.")

        # Show summary table
        summary = {k: len(v) for k, v in stats_trackers.items()}
        st.subheader("⚙️ Debug Summary")
        st.dataframe(pd.DataFrame(list(summary.items()), columns=["Category", "Count"]))

        # Detailed failure report
        fail_rows = []
        for category, lst in stats_trackers.items():
            for name, smiles in lst:
                fail_rows.append({"Protac Name": name, "SMILES": smiles, "Failure Reason": category})
        fail_df = pd.DataFrame(fail_rows)

        if not fail_df.empty:
            st.download_button("💾 Download Failure Report", fail_df.to_csv(index=False),
                            file_name="protac_failure_report.csv", mime="text/csv")
